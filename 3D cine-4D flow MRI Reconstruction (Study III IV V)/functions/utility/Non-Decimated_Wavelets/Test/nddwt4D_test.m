...
sizes = [64,64,20,10];             % Set the size of the 4D object
level = 1;                      % Set the Level of decomposition
wnames = {'db1','db3','db1','db1'};   % Set the wavelet used 
...
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','mat','precision','double');
...
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','mex','precision','double');
...
%% GPU Off Computatoin
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','gpu_off','precision','double');
...
%% GPU computation
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','gpu','precision','double');
% xGPU = gpuArray(x);
xGPU = x;
% Perform a multilevel wavelet decomposition
tic;
...
%% Matlab Single
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','mat','precision','single');
x = single(x);
% Perform a multilevel wavelet decomposition
tic;
x_trans = nddwt.dec(x,level);
...
%% GPU_off Single
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','gpu_off','precision','single');
...
%% Initialize the nd_dwt_4D class
nddwt = nd_dwt_4D(wnames,size(x),'pres_l2_norm',perserve_l2_norm,'compute','gpu','precision','single');
xGPU = gpuArray(x);
