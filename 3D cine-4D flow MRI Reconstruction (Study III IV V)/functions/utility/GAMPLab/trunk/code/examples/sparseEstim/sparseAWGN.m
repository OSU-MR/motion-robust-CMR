...
% Parameters
nx = 2000;         % Number of input components (dimension of x)
nz = 1000;         % Number of output components (dimension of y)
...
A = single(A);
y = single(y);
Alin = MatrixLinTrans(A);
outputEst = AwgnEstimOut(y, wvar);

% Set options
opt = GampOpt();
opt.legacyOut = false;
opt.xhat0 = zeros(nx,1,'single');

tic
[estFin,optFin,estHist] = gampEst(inputEst, outputEst, Alin, opt);
toc
xhat2 = estFin.xhat;
mseGAMP = 20*log10( norm(x-xhat2)/norm(x));
fprintf(1,'GAMP: MSE = %0.10f dB\n', mseGAMP);