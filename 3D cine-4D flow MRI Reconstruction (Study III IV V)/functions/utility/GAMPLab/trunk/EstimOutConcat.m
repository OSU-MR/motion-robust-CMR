classdef EstimOutConcat < EstimOut
    ...
    methods
        ...
        function obj = EstimOutConcat(estimArray, nz, precision,compute)    
            obj = obj@EstimOut;
...
%             zhat = zeros(obj.ind(nelem+1)-1,obj.ncol,obj.precision);
%             zvar = zeros(obj.ind(nelem+1)-1,obj.ncol,obj.precision);
            
            if strcmpi(obj.compute,'gpu')
                zhat = zeros(obj.ind(nelem+1)-1,obj.ncol,obj.precision,'gpuArray');   
                zvar = zeros(obj.ind(nelem+1)-1,obj.ncol,obj.precision,'gpuArray');
            else
                zhat = zeros(obj.ind(nelem+1)-1,obj.ncol,obj.precision);
                zvar = zeros(obj.ind(nelem+1)-1,obj.ncol,obj.precision);
            end
...
%                 if strcmpi(obj.compute,'gpu')
                [zhat(I,:), zvar(I,:)] = obj.estimArray{i}.estim(phat(I,:),pvar(I,:));
%                     %zhati = gather(zhati); zvari = gather(zvari);
%                 else 
...
%                 zvar(I,:) = zvari;
            ...
            if strcmpi(obj.compute,'gpu')
                zhat = gather(zhat); zvar = gather(zvar);
            end
