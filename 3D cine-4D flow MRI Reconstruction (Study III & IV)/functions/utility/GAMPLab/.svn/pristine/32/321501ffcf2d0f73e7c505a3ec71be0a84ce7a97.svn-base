classdef ncCAwgnEstimOut < EstimOut
    ...
    methods
        ...
        function var0 = tune(obj, phatfix, pvar)
            ...
            step = 1.0; %[dflt=0.5]
            maxIt = 1;
%var0_ = [mean(var0),zeros(1,maxIt)];
            for i=1:maxIt,
                if 1
                    dI0 = 0.5./(var0+pvar); % approximation for small var0+pvar 
                else
                    dv = 0.01*var0; 
...
%figure(1); plot(var0_,'.-'); pause  % to check step and maxIt

%dI0_approx = 0.5./(var0+pvar); mean((dI0-dI0_approx)./dI0_approx)  % to check derivative approximation
        ...
        function ll = logScale(obj,Axhat,pvar,phat)
            ...
            if ~(obj.maxSumVal)
                ...
                opt.phat0 = Axhat; % SEEMS THAT Axhat WORKS BETTER
                opt.alg = 1; % approximate newton's method
              opt.maxIter = 25; % [dftt=10] INCREASE FOR MORE ACCURACY
                opt.tol = 1e-6; 
