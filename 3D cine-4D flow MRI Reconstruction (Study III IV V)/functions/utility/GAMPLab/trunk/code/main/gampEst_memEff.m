function [out1,out2,out3,out4,out5,out6,out7,out8,out9] = ...
    gampEst_memEff(scaEstIn, scaEstOut, A, opt)
...
if isa(gather(xhat),'single')
%     display('using single')
    shat = zeros(m,s,'single');      % default value is zero
    svar = nan(m,s,'single');        % will test for NaN later
    xhatDamp = nan(n,s,'single');    % will test for NaN later
    pvarOpt = nan(m,s,'single');     % will test for NaN later 
    rvarOpt = nan(n,s,'single');     % will test for NaN later 
    A2xvarOpt = nan(m,s,'single');   % will test for NaN later 
else
%     display('using double')
    ...
    A2xvarOpt = nan(m,s);   % will test for NaN later zzz
end

% if isa(xhat,'gpuArray')
% %     display('using GPU')
%     shat = gpuArray(shat);      % default value is zero
%     svar = gpuArray(svar);        % will test for NaN later
%     xhatDamp = gpuArray(xhatDamp);    % will test for NaN later
%     pvarOpt = gpuArray(pvarOpt);     % will test for NaN later 
%     rvarOpt = gpuArray(rvarOpt);     % will test for NaN later 
%     A2xvarOpt = gpuArray(A2xvarOpt);   % will test for NaN later 
% end
...
% Replace the stepMax adaptation quantities with user-provided values       ******** (Do not worry about)
failCount = 0;
if ~isempty(opt.failCount0)
    failCount = opt.failCount0;
end

% If the mean-removal option is set, create an augmented system             ******** (Do not worry about)
...
% If uniform-variance mode is requested by the user, implement it by        ******** (Do not worry about)
...
% If desired, automatically set xvar                                        ******** (Do not worry about)
if (opt.xvar0auto)
    iopt.maxIter = 100;
    iopt.stepsize = 0.25;
    iopt.regularization = 0;
    iopt.tol = 1e-4;
    iopt.debug = false;
    ...
    for t=1:10
        ...
        [phat,zhat,zvar] = estimInvert(scaEstOut,Axhat0,pvar,iopt);
           %NRz = norm(zhat(:)-Axhat0(:))/norm(Axhat0(:))
        svar = (1-zvar./pvar)./pvar;
        rvar = max(rvarMin, 1./(A.multSqTr(svar)));
        xvarOld = xvar;
        [~,xhat,xvar] = estimInvert(scaEstIn,xhat0,rvar,iopt);
           %NRx = norm(xhat(:)-xhat0(:))/norm(xhat0(:))
        if norm(xvar(:)-xvarOld(:))<norm(xvar(:))*iopt.tol, break; end;
    end
    xhat = xhat0;
end
...
%     rhat = gpuArray(rhat);              
%     rvar = gpuArray(rvar);               
%     xhatFinal = gpuArray(xhatFinal);
% end

if (saveHist)                                                               %******** (Do not worry about)
...
% If using BB stepsize adaptation, compute column norms for use in scaling  ******** (Do not worry about)
if bbStep
...
%             tic;
            if isa(xhat,'gpuArray')
                valOut = gather(sum(sum(scaEstOut.logLike(gpuArray(Axhat),gpuArray(pvar)))));
            else
...
%   pass = (~adaptStep) || (step <= stepMin) || isempty(valMin) || (val >= valMin);
    ...
    % Save the stepsize and pass/fail result if history requested           ******** (Do not worry about)
    if itSaveHist
    ...
    if (pass)
        ...
        if (it>1) && (stop==false)
            if (norm(xhatPrevFinal(:) - xhatFinal(:)) / norm(xhatFinal(:)) < tol)
                stop = true;
            elseif customStop==1 
                stop = stopFcn(val, xhatFinal, xhatPrevFinal, Axhat);
            elseif customStop==2 
                S = struct(...
                    'it',it,...
                    'val',val,'xhatPrev',xhatPrevFinal,'Axhat',Axhat, ...
                    'xhat',xhatFinal,'xvar',xvar,...
                    'rhat',rhat,'rvar',rvar,...
                    'phat',phat,'pvar',pvar,...
                    'zhat',zhat,'zvar',zvar,...
                    'shat',shat,'svar',svar ...
...
%         if isa(xhat,'gpuArray')
% %             tic;
%             [zhat,zvar] = scaEstOut.estim(gpuArray(phat),gpuArray(pvarRobust));
% %             toc;
%         else
%             tic;
...
%         end
        ...
        % Compute new BB Step size if requested                             *********** (Do not worry about)
        if bbStep && it > 2
    ...
    if isa(xhat,'gpuArray')
%         tic;
        rvar = gather(1./A.multSqTr(gpuArray(svar)));   % rvar = 1./((A.^2)*svar)
        svar = gather(svar);
%         tic;
    else
        rvar = 1./A.multSqTr(svar);
    end
    ...
    if isa(xhat,'gpuArray')
%         tic;
        rhat = xhatDamp + rvar.*(A.multTr(gpuArray(shat))); % rhat = xhat + rvar.*(A'*shat)
        rhat = gather(rhat);
%         toc;
    else
%         tic;
...
%         toc;
    ...
    if isa(xhatFinal,'gpuArray')
        xhat = gpuArray(xhat);
        xhatDamp = gather(xhatDamp);
        xhatDampOpt = gather(xhatDampOpt);
    end

end % main loop


% (Here if we could gather data back onto CPU since steps b/w ReVEAL are
% relatively easy)
