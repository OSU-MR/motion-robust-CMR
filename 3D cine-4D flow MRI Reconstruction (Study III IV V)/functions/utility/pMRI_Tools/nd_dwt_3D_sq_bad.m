...
classdef nd_dwt_3D_sq
    ...
    methods
        % Constructor Object
        function obj = nd_dwt_3D_sq(wname,sizes,varargin)
            % Set Image size
            if length(sizes) ~=3
    ...
    methods (Access = protected,Hidden = true)
        ...
        % Decomposition Filters
            ...
            % Square the filters
            dec_LLL = abs(dec_LLL).^2;
            dec_HLL = abs(dec_HLL).^2;
            dec_LHL = abs(dec_LHL).^2;
            dec_HHL = abs(dec_HHL).^2;
            dec_LLH = abs(dec_LLH).^2;
            dec_HLH = abs(dec_HLH).^2;
            dec_LHH = abs(dec_LHH).^2;
            dec_HHH = abs(dec_HHH).^2;
