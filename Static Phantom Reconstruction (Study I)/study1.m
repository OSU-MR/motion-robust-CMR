...
% CS parameters
p.mu_cs  = 1e-1; % langrange multiplier hyperparameter
p.lam_cs = 1e-2*[1e-2, ones(1,3)]; % 4 regularization parameters for regularization
...
% SO parameters
p.mu_so  = 2e-2; % langrange multiplier hyperparameter
p.lam1_so = 1e-2*[1e-2, ones(1,3)]; % 4 regularization parameters for regularization 
...
% CORe parameters
p.mu1_core  = 2e-2; % langrange multiplier hyperparameter
p.mu2_core  = 2e-1; % langrange multiplier hyperparameter
p.lam1_core = 1e-2*[1e-2, ones(1,3)]; % 4 regularization parameters for regularization
...
yscl = ymax/max(abs(y(:))); % scaling factor
y = yscl*y; 
x = yscl*x;
xmax = max(abs(x(:)));
noise=(sig1*ymax*(randn(size(y)) + 1j*randn(size(y)))).*p.s; % Gaussian noise to be added
yn = y + (sig1*ymax*(randn(size(y)) + 1j*randn(size(y)))).*p.s; % adding Gaussian noise to data
...
yn(:,ind) = yn(:,ind) + sig2*ymax*(randn(p.N(2),no_arr(iter)) + 1j*randn(p.N(2),no_arr(iter))); % adding outliers to data
...
title(['ZF, nmse: ' num2str( ssim(real(x_zf),real(x)),3) ])

subplot(2,k,9); imagesc(5*abs(x_cs - x),[0,xmax]); axis('image','off'); colormap('gray');
title(['CS, avg ssim: ' num2str((mean_ssim_cs),3) '+- std:' num2str((std_ssim_cs),3) ])

subplot(2,k,10); imagesc(5*abs(x_rr - x),[0,xmax]); axis('image','off'); colormap('gray');
title(['RR, avg ssim: ' num2str((mean_ssim_rr),3) '+- std:' num2str((std_ssim_rr),3) ])

subplot(2,k,11); imagesc(5*abs(x_so - x),[0,xmax]); axis('image','off'); colormap('gray');
title(['SO, avg ssim: ' num2str((mean_ssim_so),3) '+- std:' num2str((std_ssim_so),3) ])

subplot(2,k,12); imagesc(5*abs(x_core - x),[0,xmax]); axis('image','off'); colormap('gray');
title(['CORe, avg ssim: ' num2str((mean_ssim_core),3) '+- std:' num2str((std_ssim_core),3) ])
