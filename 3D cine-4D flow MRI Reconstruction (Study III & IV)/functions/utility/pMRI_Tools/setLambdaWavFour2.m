function [ lambda_vec ] = setLambdaWavFour2( sizes,lambda )
...
%     lambda_band(8) = 9*k*lambda;
    ...
    lambda_vec = zeros([sizes,9]);
    for ind = 1:9
       lambda_vec(:,:,:,ind) = lambda_band(ind);
    end
    lambda_vec(:,:,1:2,9) = 0*lambda_vec(:,:,1:2,9);
    lambda_vec(:,:,end-1:end,9) = 0*lambda_vec(:,:,end-1:end,9);
