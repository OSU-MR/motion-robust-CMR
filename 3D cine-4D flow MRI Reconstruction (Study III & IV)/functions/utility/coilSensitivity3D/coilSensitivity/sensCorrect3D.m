function [xc, sc] = sensCorrect3D(x, s, option)
% option = 1;
...
[~,~,~,Nc,~] = size(s); % Nc: number of coils (s:kx,ky,kz,coil,set)
...
elseif option == 1
    ...
    map = exp(-1j*phs);
    xc = x.*map;
    sc = bsxfun(@times,s,repmat(permute(conj(map),[1 2 3 5 4]),[1,1,1,Nc,1]));%kx,ky,kz,coil,map-set
