function [xhat,maps,zhat] = pMRIRED(kdata,samp,wvar,lamRED,lamW,denoise,maps)
...
switch lower(denoise)
    case ('nd_wavelets')
        ...
        denoiser = @(x) shrinkWave3D(x,size(samp),W,lamW);
...
[xhat,zhat, ~] = admmRED(x0(:), kdata, pMRI, lamRED, denoiser);
