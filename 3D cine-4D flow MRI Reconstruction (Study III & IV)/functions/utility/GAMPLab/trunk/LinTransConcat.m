classdef LinTransConcat < LinTrans 
    ...
    methods 
         ...
         % scaling is optional
         function obj = LinTransConcat(linTransArray,scaling,precision,compute)
            obj = obj@LinTrans;
            ...
            for k=1:length(obj.lta)
                if isobject( obj.lta{k} ) && ismethod(obj.lta{k},'multSqTr');% this object appears to be a usable object already 
                else
...
%             y=zeros(obj.sz(1),size(x,2),obj.precision);
            if strcmpi(obj.compute,'gpu')
                y=zeros(obj.sz(1),size(x,2),obj.precision,'gpuArray');
%                y = gpuArray(y);
            else
                y=zeros(obj.sz(1),size(x,2),obj.precision);
            end
        ...
        function z = multSq(obj,x)
            if strcmpi(obj.compute,'gpu')
                z=zeros(obj.sz(1),size(x,2),obj.precision,'gpuArray');
            else
                z=zeros(obj.sz(1),size(x,2),obj.precision);
            end
...
%             x=zeros(obj.sz(2),size(z,2),obj.precision);
            if strcmpi(obj.compute,'gpu')
                x=zeros(obj.sz(2),size(z,2),obj.precision,'gpuArray');
            else
                x=zeros(obj.sz(2),size(z,2),obj.precision);
            end
            i=1;
