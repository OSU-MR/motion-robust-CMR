    ...
    methods
        ...
        function [zmean, zvar] = estim(obj, zmean0, zvar0)
            ...
            if isa(zmean0,'gpuArray')
                ...
                s = obj.scale;
%                 gain = arrayfun(@(a,b,c)(conj(a).*b./((abs(a).^2).*b + c)),s,zvar0,obj.wvar);
                zmean = conj(s)*zvar0./((abs(s)^2)*zvar0 + obj.wvar).*(obj.y-s*zmean0) + zmean0;
                clear zmean0
...
%                 predErr = (abs(obj.y-s*zhat).^2 + (abs(s)^2)*zvar)./wvar1;
                ll = -((abs(obj.y-s*zhat).^2 + (abs(s)^2)*zvar)./wvar1);
            else
%                 predErr = (abs(obj.y-s*zhat).^2)./wvar1;
                ll = -((abs(obj.y-s*zhat).^2)./wvar1);
            end
