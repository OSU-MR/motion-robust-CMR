...
classdef harr_nddwt_3D
    ...
    methods
        % Constructor Object
        function obj = harr_nddwt_3D(wname,sizes,varargin)
            % Set Image size
            if length(sizes) ~=3
...
%             level = ceil(size(x,4)/8); % Rizwan: commented out
...
%             y(:,:,:,1) = 1/sqrt(numel(x_f))*(x_f.*obj.f_dec.LLL); %??? if the FFT of LLL is desired instead--part I
%             y(:,:,:,1) = 1/sqrt(8)*1/sqrt(numel(x_f(1,1,:)))*ifft(ifft((x_f.*obj.f_dec.LLL),[],1),[],2); %??? if the temporal FFT of LLL is desired instead--part I
...
% %             y = ifftn(squeeze(ifftn(x_f(:,:,:,1))).*sqrt(numel(x_f(:,:,:,1))).*conj(obj.f_dec.LLL)); %??? if the FFT of LLL is desired instead--part II
% %             y = sqrt(8)*ifftn(squeeze(ifft(x_f(:,:,:,1),[],3)).*sqrt(numel(x_f(1,1,:,1))).*conj(obj.f_dec.LLL)); %??? if the temporal FFT of LLL is desired instead--part II 
